...
extern void progress_update(struct progress_bar *prog, unsigned long long copied, unsigned long long current, int done)
{
	...
	fprintf(stderr, "\r%80c\rElapsed: %s, Remaining: %s, Completed: %6.2f%%", clear_buf, prog_stat.Eformated, prog_stat.Rformated, prog_stat.percent);

	if((prog->flag == 2) || (prog->flag == 3))
	    fprintf(stderr, ", %6.2f%s/min,", prog_stat.speed, prog_stat.speed_unit);
	if(prog->flag == 2)
	    fprintf(stderr, "\n\r%80c\rcurrent block: %10Lu, total block: %10Lu, Complete: %6.2f%%%s\r", clear_buf, current, prog->total, prog_stat.total_percent, "\x1b[A");
    } else {

	fprintf(stderr, "\r%80c\rElapsed: %s, Remaining: %s, Completed: 100.00%%", clear_buf, prog_stat.Eformated, prog_stat.Rformated);
	if((prog->flag == 2) || (prog->flag == 3))
	    fprintf(stderr, ", Rate: %6.2f%s/min,", prog_stat.speed, prog_stat.speed_unit);
	if(prog->flag == 2)
	...
	if(prog->flag == 2)
	    fprintf(stderr, "Ave. Rate: %6.1f%s/min, ", prog_stat.speed, prog_stat.speed_unit);
        fprintf(stderr, "%s", "100.00% completed!\n");
    }
}
...
extern void Ncurses_progress_update(struct progress_bar *prog, unsigned long long copied, unsigned long long current, int done)
{
	...
	prog->resolution_time = time(0);
        ...
        mvwprintw(p_win, 0, 0, "Elapsed: %s Remaining: %s " , prog_stat.Eformated, prog_stat.Rformated);
	if((prog->flag == IO) || (prog->flag == NO_BLOCK_DETAIL))
	    mvwprintw(p_win, 0, 40, "Rate: %6.2f%s/min", prog_stat.speed, prog_stat.speed_unit);
	if (prog->flag == IO)
	    mvwprintw(p_win, 1, 0, "Current Block: %llu  Total Block: %llu " , current, prog->total);
	...
	wrefresh(p_win);
        ...
        mvwprintw(p_win, 0, 0, "Total Time: %s Remaining: %s ", prog_stat.Eformated, prog_stat.Rformated);
	if((prog->flag == IO) || (prog->flag == NO_BLOCK_DETAIL))
	    mvwprintw(p_win, 1, 0, "Ave. Rate: %6.2f%s/min", prog_stat.speed, prog_stat.speed_unit);
