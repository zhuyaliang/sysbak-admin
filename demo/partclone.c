...
void print_readable_size_str(guint64 size_byte, char *new_size_str)
{
    ...
    if (size_byte == 0)
        snprintf(new_size_str, 11, "%" G_GUINT64_FORMAT, size_byte);
 ...
 *
 */
guint64 cnv_blocks_to_bytes(guint64 block_offset, unsigned int block_count, unsigned int block_size, const image_options* img_opt)
{

    guint64 bytes_count = block_count * block_size;
...
/// get partition size
guint64 get_partition_size(int* ret)
{
    guint64 dest_size = 0;
    ...
    if (!fstat(*ret, &stat))
    {
        ...
        else
        {
            ...
            if (ioctl(*ret, BLKGETSIZE, &dest_block) >= 0)
                dest_size = (guint64)(dest_block * 512);
...
int check_size(int* ret, guint64 size)
{
...
unsigned long get_checksum_count(guint64 block_count, const image_options *img_opt)
{
...
guint64 get_bitmap_size_on_disk(const file_system_info* fs_info, const image_options* img_opt, cmd_opt* opt)
{
    guint64 size = 0;
...
/// get free space
guint64 get_free_space(char* path)
{
    ...
    dest_size = (guint64)stvfs.f_frsize * stvfs.f_bfree;
    if (!dest_size)
        dest_size = (guint64)stvfs.f_bsize * stvfs.f_bfree;
...
/// check free space
void check_free_space(char* path, guint64 size)
{
...
void check_mem_size(file_system_info fs_info, image_options img_opt, cmd_opt opt)
{
    ...
    const guint64 bitmap_size = BITS_TO_BYTES(fs_info.totalblock);
    ...
    const guint64 raw_io_size = buffer_capacity * block_size;
    guint64 cs_size = 0, needed_size = 0;
    void *test_bitmap, *test_read, *test_write;

    if (img_opt.checksum_mode != CSM_NONE)
    {

        guint64 cs_in_buffer = buffer_capacity / blkcs;
...
void load_image_bitmap_bits(int* ret, cmd_opt opt, file_system_info fs_info, unsigned long* bitmap)
{
    ...
    guint64 r_size, bitmap_size = BITS_TO_BYTES(fs_info.totalblock);
    uint32_t r_crc, crc;
...
void load_image_bitmap_bytes(int* ret, cmd_opt opt, file_system_info fs_info, unsigned long* bitmap)
{
    ...
    guint64 size, r_size, r_need;
    char buffer[16384];
    guint64 offset = 0;
    guint64 bused = 0, bfree = 0;
    int i, debug = opt.debug;
    int err_exit = 1;
...
int write_block_file(char* target, char *buf, guint64 count, guint64 offset, cmd_opt* opt)
{
    long long int i;
    int debug = opt->debug;
    guint64 size = count;
    extern guint64 rescue_write_size;
    int flags = O_WRONLY | O_LARGEFILE | O_CREAT ;
    int torrent_fd = 0;
    char *block_filename = malloc(PATH_MAX + 1);
    log_mesg(0, 0, 0,debug,  "offset %lld, size %lld\n", offset, count);
    sprintf(block_filename,"%s/%032" G_GUINT64_FORMAT "x", target, offset);
...
int io_all(int *fd, char *buf, guint64 count, int do_write, cmd_opt* opt)
{
    long long int i;
    int debug = opt->debug;
    guint64 size = count;
...
void rescue_sector(int *fd, guint64 pos, char *buff, cmd_opt *opt)
{
...
void print_file_system_info(file_system_info fs_info, cmd_opt opt)
{
    ...
    guint64 total = fs_info.totalblock;
    guint64 used  = fs_info.usedblocks;
    int debug = opt.debug;
    char size_str[11];
